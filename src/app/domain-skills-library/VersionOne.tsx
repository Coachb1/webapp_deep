  ...
  return (
    <>
      ...
      <MaxWidthWrapper className="flex pt-20 flex-col items-center justify-center text-center">
        ...
        <p className="mt-4">
          Thousands of Microlearning embedded roleplay simulations!
        </p>
        {/* Category buttons */}
        {Object.keys(data).length > 0 && (
          <>
            {Object.entries(tabTypeInformation)
              .sort(([aKey], [bKey]) => aKey.localeCompare(bKey))
              .map(([tabType, categories]) => {
                const badgeText =
                  tabType === "undefined"
                    ? "Roleplay Observation (Difficult Conversations)"
                    : tabType.charAt(0).toUpperCase() + tabType.slice(1);

                const badgeColor =
                  tabTypeColors[tabType.toLowerCase()] || "bg-gray-200";

                return (
                  <div key={tabType} className="relative mt-10">
                    {/* Centered, nowrap Badge */}
                    <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 z-10">
                      <Badge
                        className={`px-4 rounded-md text-gray-800 hover:bg-gray-300 capitalize whitespace-nowrap max-w-fit ${badgeColor}`}
                      >
                        {badgeText}
                      </Badge>
                    </div>
                        ...
                        <div className="flex flex-wrap justify-center gap-3 mt-2">
                          {(categories as any[]).map((categoryObj) => (
                            <Button
                              ...
                              onClick={() =>
                                scrollToView(getUniqueId(categoryObj.category))
                              }
                            >
        ...
        <div className="flex flex-row flex-wrap justify-center mt-2 z-[2] gap-2 p-3 rounded-md">
          ...
          <Badge
            className={`mt-6 -mb-6 px-4 z-10 rounded-md text-gray-800 hover:bg-gray-300 bg-blue-200`}
          >
            Others
          </Badge>
          ...
          <div className="w-full max-w-4xl">
            ...
            >
              <div className="flex flex-wrap justify-center gap-3 mt-2">
                {!userInfo.restrictedFeatures?.includes(
                  "Assigned-scenarios"
                ) && (
                  <Button
                    onClick={() => {
                  ...
                  >
                    Assigned Simulations <History className="h-4 w-4 ml-2" />
                  </Button>
                )}
              </div>
            </Div>
      ...
      <div className="flex pt-2 flex-col items-center justify-center text-center">
        {Object.entries(data).map(([category, simulations], index) => (
          <div
            className="w-full scroll-mt-[3rem]"
                ...
                <div className="relative isolate mx-auto">
                  <div>
                    <div className="mx-auto w-full mt-4 max-sm:w-[100%] z-50">
                      <div
                        id={index == 0 ? "nav3" : String(index)}
                        className="rounded-xl bg-white ring-1 ring-inset ring-gray-900/10 lg:rounded-2xl max-sm:w-[100%]"
                      >
                      ...
                      <div>
                        <div className="mx-auto w-full mt-4 max-sm:w-[100%] z-50">
                          <div className="rounded-xl bg-white ring-1 ring-inset ring-gray-900/10 lg:rounded-2xl max-sm:w-[100%]">
                            {requestedTestsData?.assignedscenarios.length >
                            0 ? (
                              <LibraryTestsAccordian
                                attemptedTests={attemptedTests}
                                tests={requestedTestsData?.assignedscenarios}
                                type={"assigned"}
                              />
                            ) : (
                              <p className="text-sm max-sm:text-xs text-gray-600 py-4">
                                You don't have any assigned scenarios yet.
                              </p>
                            )}
                          </div>
                        </div>
